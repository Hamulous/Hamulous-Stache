...
with open("Propertysheets.json", "r") as f:
    property_data = json.load(f)

def transform_costumes(costume):
    costume_id = costume.get("CostumeID")
    plant_type_name = clean_string(costume.get("PlantTypeName").lower())  # Cleaned
    friendly_name = clean_string(costume.get("FriendlyName").lower())  # Cleaned
    ...
    if isinstance(data, dict):
        for key, value in data.items():
            if isinstance(value, (dict, list)):
                ...
                if key == "objclass" and value == "CostumePropertySheet":
...
with open("market_schedule_output.json", "w") as f:
